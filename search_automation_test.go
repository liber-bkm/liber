...
func TestSearchMatches(t *testing.T) {
	s := searchStore()
	var cfg Config
	if got := idsOf(s.Search(cfg, "alpha", SearchFields{}, false)); !equalInts(got, []int{1, 2, 3}) {
		t.Errorf("full search = %v", got)
	}
	if got := idsOf(s.Search(cfg, "alpha", SearchFields{Title: true}, false)); !equalInts(got, []int{1}) {
		t.Errorf("title search = %v", got)
	}
	if got := idsOf(s.Search(cfg, "todo", SearchFields{}, false)); !equalInts(got, []int{2}) {
		t.Errorf("tag search = %v", got)
	}
	if got := idsOf(s.Search(cfg, "", SearchFields{}, false)); !equalInts(got, []int{1, 2, 3}) {
